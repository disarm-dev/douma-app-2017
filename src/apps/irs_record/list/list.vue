...
<script>
  ...
  export default {
    ...
    computed: {
      tasksWithDistance() {
        if (this.userCoords) {
          return this.$store.state.irs_record.tasks.map((task) => {
          ...
          return this.$store.state.irs_record.tasks
        }
      ...
      taskStat() {
        const total = this.$store.state.irs_record.tasks.length
      ...
      userCoords() {
        const userCoordsMarker = window.douma.data.irs_record.userCoordsMarker
    ...
    methods: {
      ...
      setActiveTask(action) {
        this.$store.commit('irs_record:setActiveTask', action)
      },
      actionClass(task) {
      ...
      findClosestTask() {
        ...
        this.$store.dispatch('irs_record:setActiveTaskByOSMId', closest_osm_id)
      },
      finishTasks() {
        this.$store.dispatch("irs_record:finishTasks")
      }
    }
  }
</script>
