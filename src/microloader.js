...
MicroLoader.startServiceWorker = function() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    navigator.serviceWorker.register(SW_SCRIPT_NAME, {scope: '/'}).then(registration => {
...
MicroLoader.loadInitialAssets = function() {
  ...
  for (let asset of this.state.currentConfig.assets.initial) {
    ...
    if (asset.endsWith('.js')) {
      ...
      let fullName = '/' + versionedAssetName(asset, this.state.currentConfig)
      console.log('[ML] Adding script tag for path:', fullName)
      script.src = fullName
